...
TOOL_DF: pd.DataFrame | None = None
...
def inspect_dataset_schema() -> str:
    ...
    return dataframe_schema(TOOL_DF)
...
def analyze_business_data(
    ...
    if TOOL_DF is None:
        return json.dumps({"error": "No dataset is loaded."})
    ...
    result_df = execute_sql_on_dataframe(TOOL_DF, safe_sql)
    summary = summarize_result(result_df, TOOL_DF)
    payload = {
        "intent": plan["intent"],
        "sql": safe_sql,
